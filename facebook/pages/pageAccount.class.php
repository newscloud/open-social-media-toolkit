...
class pageAccount {
	...
	function fetch($option='settings') {
		...
		{				
			...
			switch ($option) {
				...
				break;
				case 'address':
					$inside.=$this->fetchAddressSettings();
				break;
				case 'subscribe':
	...
	function buildSubNav($currentSub='settings') 
	{
		if ($currentSub=='' OR is_null($currentSub)) $currentSub='settings';
		$pages = array();		
		$pages['settings'] = 'General';
		$pages['subscribe']='Subscriptions';
		if (ENABLE_ACTION_CHALLENGES) {
			$pages['address']='Mailing Address';			
		}
		 ...
		 {		 	
	 		...
	 		$tabs.='<li '.$clsName.'><a id="subtab'.$pagename.'" href="?p=account&o='.$pagename.'" '.($currentSub==$pagename?'class="selected"':'').'>'.$pages[$pagename].'</a></li>';	
		 	$i++;
		 }
	...
	function fetchAddressSettings() {
		...
		{
			$fdata = $account->validateLocationData(false); // validate for existing account
								
			if ($fdata->result)
			{
				$fdata = $account->processLocationUpdate($fdata);
			} 
			
			if (!$fdata->result) // can be either validation failure or processing failure
			{
				$code .= $account->buildAccountAddressForm($fdata, $fdata->alert);
				
			} else
			{
				// success -- just redisplay w/ updated data
				$code .= "Update successful..."; 
				$this->facebook->redirect(URL_CANVAS.'?p=account&o=address&message=success');
			}	
		} else {
...
*/
	...
	function fetchSubscribeSettings()
	{
		...
		if (isset($_GET['step']) && $_GET['step'] == 'submit')
		{			
			$fdata->noCommentNotify = $_POST['noCommentNotify'] == 'on' ? 1 : 0;
			$fdata->rxFeatures = $_POST['rxFeatures'] == 'on' ? 1 : 0;
			$fdata->rxMode	= $_POST['rxMode'];
			$fdata = $account->updateSubscriptions($fdata);
			$code .= "Update successful..."; 
			$this->facebook->redirect(URL_CANVAS.'?p=account&o=subscribe&message=success');
		} else
		{	
			$fdata = $account->initFormDataFromDatabase($this->page->session->userid);

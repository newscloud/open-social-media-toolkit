...
class newsroomPage extends XHTMLpage {
	var $header = '';
	var $sidebar = '';
	var $content = '';
	var $footer = '';
...
class XHTMLpage {
	...
	function pkgScripts($page='default',$scripts) {
		$scripts=array_merge(array(PATH_PHP_SCRIPTS.'/common.js'),$scripts);
	...
	function pkgStyles($page='default',$sheets) {
		...
		$sheets=array_merge(array(PATH_PHP_STYLES.'/layout.css',PATH_PHP_STYLES.'/ads.css'),$sheets);
   ...
   function pkgWrite($sFilename, $sCode) {
	...
	function _genStylesheets()
	{
		...
		$ret.='<link rel="icon" href="'.URL_HOME.'?p=img&img=favicon.gif" type="image/gif" />';
		return $ret;
	}

...
class MostDiscussedStories extends BasePod {
	...
	function load_schema($view = 'pod') {
		...
		$schema['labels'][] = array('key' => 'cnt', 'label' => '# of Comments');
	...
	function load_all($view = 'full', $params = false) {
		//$results = $this->model->db->query("SELECT itemid2 as itemid,title, count(Log.id) as cnt FROM Log,Content where Log.itemid2=siteContentId AND action='comment' GROUP BY itemid2 ORDER BY cnt DESC");
		$results = $this->db->query("SELECT itemid2 as itemid,title, count(Log.id) as cnt FROM Log,Content where Log.itemid2=siteContentId AND action='comment' GROUP BY itemid2 ORDER BY cnt DESC");

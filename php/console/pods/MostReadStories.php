...
class MostReadStories extends BasePod {
	...
	function load_schema($view = 'pod') {
		...
		$schema['labels'][] = array('key' => 'cnt', 'label' => '# of Readers');
	...
	function load_all($view = 'full', $params = false) {
		//$results = $this->model->db->query("SELECT itemid,title, count(Log.id) as cnt FROM Log,Content where Log.itemid=siteContentId AND action='readStory' GROUP BY itemid ORDER BY cnt DESC");
		$results = $this->db->query("SELECT itemid,title, count(Log.id) as cnt FROM Log,Content where Log.itemid=siteContentId AND action='readStory' GROUP BY itemid ORDER BY cnt DESC");
		$stories = array();
	...
	function load_statistics($view = 'pod') {
		...
		$jsonarr['Stats']['ActionName'] = 'Most Read Stories';

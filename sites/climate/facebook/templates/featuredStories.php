<?php
	...
	function getImageLink($story = false, $size = 'large') {
		...
		if ($imageid == 0 || !($size == 'large' || $size == 'small'))
			...
			return sprintf("<a %s><img src=\"http://www.newscloud.com/images/scaleImage.php?id=%s&x=%s&y=%s&fixed=x&crop\" /></a>", $url, $imageid, ${$size.'ImageWidth'}, ${$size.'ImageHeight'});
	}
	...
	function getCaption($story) {
		...
		$tmp = $templateObj->cleanString($caption, 130);
